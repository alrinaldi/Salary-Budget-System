	...
	<script type= 'text/javascript'>
		$(document).ready(function () {
			$('#dataTable').DataTable({
				"processing": true,
				"serverSide": true,
				"ajax": "<?php echo base_url('dept/c_overtime/get_ovt')?>"
			});
			...
			$(document).delegate('.edit', 'click', function() {
				...
				var id_master_overtime = parent.children("td:nth-child(1)");
				var workcenter = parent.children("td:nth-child(2)");
				var dept = parent.children("td:nth-child(3)");
				var seksi = parent.children("td:nth-child(4)");
				...
				kontrak.html("<input type='number' id='txtKontrak' value='"+kontrak.html()+"'/>");
				gol1.html("<input type='number' id='txtGol1' value='" + gol1.html()+"'/>");
				...
				buttons.html("<button id='save'>Save</button>  <button class='delete' id='" + id_master_overtime.html() + "'>Delete</button>");
			});
			...
			$(document).delegate('#save', 'click', function() {
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url('dept/c_overtime/update_overtime')?>",
					data: 'id_master_overtime=' + id_master_overtime.html() + '&workcenter=' + workcenter.children("input[type=text]").val() + '&seksi=' + seksi.children("input[type=text]").val() + '&dept=' + dept.children("input[type=text]").val() + '&kontrak=' + kontrak.children("input[type=text]").val() + '&gol1=' + gol1.children("input[type=text]").val() + '&gol2=' + gol2.children("input[type=text]").val() + '&gol3=' + gol3.children("input[type=text]").val() + '&gol4=' + gol4.children("input[type=text]").val() + '&gol5=' + gol5.children("input[type=text]").val() + '&gol6=' + gol6.children("input[type=text]").val() + '&bulan=' + bulan.children("input[type=text]").val() + '&tahun=' + tahun.children("input[type=text]").val(),
					cache: false,
					success: function() {
						...
						kontrak.html(kontrak.children("input[type=number]").val());
						gol1.html(gol1.children("input[type=number]").val());
						...
						buttons.html("<button class='edit' id='" + id_master_overtime.html() + "'>Edit</button>  <button class='delete' id='" + id_master_overtime.html() + "'>Delete</button>");
					},
					error: function() {
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-2 text-gray-800">Overtime</h1>
		  <p class="mb-4"></p>
...
?><div class="form-group">
           ...
           <button class="btn btn-primary">Submit</button>
			...
			<div class="col-xs-6 col-md-4">
				<div class="card shadow mb-4">
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary">Set Overtime (/bln)</h6>
                </div>
                <div class="card-body">
				<form action="<?php echo base_url().'dept/c_overtime/set_ovt'?>" method='post'>
<?php
$year1=date('Y', strtotime("+1 year"));
$year2=date('Y', strtotime("+2 year"));
$year3=date('Y', strtotime("+3 year"));
...
?><div class="form-group">
  ...
  <div class="input-group-prepend">
    <span class="input-group-text" id="inputGroup-sizing-default">Set Overtime</span>
  </div>
  <input type="number" name = "jam" class="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default" required>
</div>
          ...
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h3 class="m-0 font-weight-bold text-primary">Overtime <?php  ?></h3>
			</div>
