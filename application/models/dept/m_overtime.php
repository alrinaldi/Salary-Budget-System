...
Class M_overtime extends CI_Model{
	...
	function list_overtime($depts,$tahun){
			$hsl = $this->db->query("SELECT * FROM MASTER_OVERTIME WHERE dept = '$depts' and tahun = '$tahun'");
			return $hsl;
	}

	function budget_lembur($year,$month,$dept){
		$hsl = $this->db->query("SELECT * FROM OVERTIME WHERE tahun ='$year' and bulan = '$month' and dept = '$dept' 
		and gol in(1,2,3,4,0)");
		return $hsl;
	}
